...
contract('Letter of Credit base flow', async accounts => {
    ...
    it('no respond from buyer, seller cancels bargain', async() => {
        let ZS = 0;
        //period didn't expired
        await expectThrow(letterOfCreditContract.cancelBargainSeller({ from: accs.attacker }));
        ...
        await letterOfCreditContract.cancelBargainSeller({ from: accs.seller });
        let state = await letterOfCreditContract.bargainInitializedBy.call(accs.buyer);
